...
{
    int rc = OK;
    points_dst.count = points_src.count;
    points_dst.arr_points = (point_t *) malloc(sizeof(point_t) * points_dst.count);

    if (points_dst.arr_points == nullptr)
        rc = MEMORY_ERROR;
    else
    {
        for (int i = 0; i < points_dst.count; i++)
            copy_point(points_dst.arr_points[i], points_src.arr_points[i]);
    }
    return rc;
}
